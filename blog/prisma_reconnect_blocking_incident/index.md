...
The `/health/liveliness` route is intentionally cheap and does not touch the database. The expectation was that even during a DB outage, liveliness would stay green and Kubernetes would leave the pod alone.
...
After the simulated DB outage ends, `/health/readiness` returns `db: "connected"` and live row reads from `/key/list` succeed, so reconnect works end-to-end.
